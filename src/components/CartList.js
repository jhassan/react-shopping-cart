...
const CartList = () => {
	console.log("storage products", JSON.parse(localStorage.getItem('products')));
	let productList = JSON.parse(localStorage.getItem('products'))
		const productsListResult = [...productList.reduce((r, e) => {
			let k = `${e.id}`;
			if(!r.has(k)) r.set(k, {...e, count: 1})
			else r.get(k).count++
			return r;
		}, new Map).values()]
		console.log("productsListResult", productsListResult);
  return (
    <>
                  ...
                  <thead>
                    <tr>
                      ...
                      <th>Total</th>
									...
									{productsListResult.length > 0 && (
                  <tbody>
										{productsListResult?.map(product => (
                    <tr key={product?.id}>
          ...
          <div className="row">
            <div className="col-12 col-md-6 col-lg-4">
              <div className="coupon-code-area mt-70">
                ...
                <form action="#">
                  <input readOnly type="search" name="search" placeholder="#569ab15" value="" />
